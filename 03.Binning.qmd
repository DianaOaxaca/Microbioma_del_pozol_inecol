...
Para generar los ensambles recomendamos ensamblar las lecturas con metaSPAdes y con MEGAHIT, compararlos con QUAST y con BBMap y seleccionar el mejor ensamble en términos de mayor porcentaje de mapeo, es decir utilizar el mayor número de lecturas posible y obtener contigs más largos.
...
#SBATCH -J Metabat2 
#SBATCH -t 0
#SBATCH -e outs/04.metabat48h.err
#SBATCH -o outs/04.metabat48h.out
#SBATCH --export=ALL
#SBATCH -n 4
#SBATCH -N 1
#SBATCH -p q2
...
# 01. crearemos el directorio en el que se van a guardar los resultados de este proceso.
mkdir -p results/04.metabat/48h/

# 02. Corremos metabat
metabat2 -i results/02.ensambles/48hrs.fasta -a results/03.profundidad/48hrs.mgh_depth.txt -o results/04.metabat/48h/metabat -m 1500 -t 4
```
...
[MaxBin2](https://sourceforge.net/projects/maxbin/files/) agrupa los contigs de acuerdo a la información de cobertura, composición nucleotídica, contenido de GC y genes **marcadores de copia única**.
...
#SBATHC -J Maxbin 
#SBATCH -t 0
#SBATCH -e outs/05.maxbin48h.err
#SBATCH -o outs/05.maxbin48h.out
#SBATCH --export=ALL
#SBATCH -n 4
#SBATCH -N 1
#SBATCH -p q2
...
# 01. Crea el directorio para los resultados de MaxBin2

mkdir -p results/05.maxbin/48h

# 02. Correr maxbin

run_MaxBin.pl -thread 4 -min_contig_length 1500 -contig results/02.ensambles/48hrs.fasta -abund results/03.profundidad/48hrs.mgh_depth.txt -out results/05.maxbin/48h/maxbin
```
...
# 01. Crea el directorio para los resultados de vamb
mkdir -p results/06.vamb

# 02. Correr vamb
vamb --fasta results/02.ensambles/48hrs.fasta --bamfiles results/03.profundidad/48hrs_sorted.bam --minfasta 200000 --outdir results/06.vamb/48h -p 8 -m 1500
```
...
En este taller no vamos a correr CheckM para los resultados de cada *binneador* porque uno de los programas de refinamiento que usaremos ya lo corren de forma interna, sin embargo, es útil correrlo y recomendamos hacerlo en la vida real para tener una idea clara sobre la calidad de los bins que vamos obteniendo.
...
``` bash
nano src/06.1.checkm_48h.slurm
```
...
#SBATHC -J checkm 
#SBATCH -t 0
#SBATCH -e outs/06.1.checkm.err
#SBATCH -o outs/06.1.checkm.out
#SBATCH --export=ALL
#SBATCH -n 6
#SBATCH -N 1
#SBATCH -p q2
...
# 01. Indicamos la ubicación de la base de datos
export CHECKM_DATA_PATH=/lustre/databases/metagenomica/checkM/

# 02. Ejecutamos checkm
checkm lineage_wf results/06.vamb/48h/bins/ results/06.vamb/48h/checkm -x fna -t 6 -f results/06.vamb/48h/checkm_vamb.tsv
```
...
``` bash
mkdir -p results/07.binning_refiner/48hbins/{metabat,maxbin,vamb}
```

**Ejercicio**. Crea ligas simbólicas de los *bins* generados por cada herramienta en el directorio que corresponde.

:::: callout-tip
## Ejercicio: Prepara los datos

Crea ligas simbólicas de los *bins* generados por cada herramienta en el directorio que corresponde de acuerdo a los directorios que acabamos de crear.

::: {.callout-note collapse="true"}
#metabat

`cd results/07.binning_refiner/48hbins/metabat/`

`ln -s ../../../04.metabat/48h/*.fa .`

#maxbin

`cd ../maxbin/`

`ln -s ../../../05.maxbin/48h/*.fasta .`

#vamb

`cd ../vamb/`

`ln -s ../../../06.vamb/48h/bins/*.fna .`
:::
::::

Regresa al directorio de resultados de Binning_refiner para ejecutar el programa dentro de ese directorio.

``` bash
# usa pwd y cd para ubicarte
```
...
``` bash
srun --mem 1G -n1 -p q2 Binning_refiner -i 48hbins/ -m 200 -p 48h
```
...
``` bash
cat results/07.binning_refiner/48h_Binning_refiner_outputs/48h_sources_and_length.txt
```
...
#SBATHC -J dastool
#SBATCH -t 0
#SBATCH -e outs/08.dastool48h.err
#SBATCH -o outs/08.dastool48h.out
#SBATCH --export=ALL
#SBATCH -n 6
#SBATCH -N 1
#SBATCH -p q2
...
# 01. Crear el directorio para los resultados
mkdir -p results/08.dastool/48h/

# 02. DASTool necesita como entrada un archivo tabular con información de los resultados de cada programa de binning.
Fasta_to_Contig2Bin.sh -i results/04.metabat/48h/ -e fa > results/08.dastool/48h/48hrs_metabat.dastool.tsv
Fasta_to_Contig2Bin.sh -i results/05.maxbin/48h/ -e fasta > results/08.dastool/48h/48hrs_maxbin.dastool.tsv
Fasta_to_Contig2Bin.sh -i results/06.vamb/48h/bins/ -e fna > results/08.dastool/48h/48hrs_vamb.dastool.tsv

# 03. Ya que tenemos los archivos tsv podemos empezar con el refinamiento!!
DAS_Tool -i results/08.dastool/48h/48hrs_metabat.dastool.tsv,results/08.dastool/48h/48hrs_maxbin.dastool.tsv,results/08.dastool/48h/48hrs_vamb.dastool.tsv -l metabat,maxbin,vamb -c results/02.ensambles/48hrs.fasta -o results/08.dastool/48h/48h -t 6 --write_bins
```
...
``` bash
mkdir -p results/09.drep/48h/bins
```

Copia los bins que queremos comparar dentro del directorio que acabamos de crear. En este caso los generados por ambos refinadores.

Con las siguientes lineas podemos copiar los bins:

``` bash
DASTOOL="results/08.dastool/48h/48h_DASTool_bins"
BR_BINS="results/07.binning_refiner/48h_Binning_refiner_outputs/48h_refined_bins"
DREP_BINS="results/09.drep/48h/bins"

for i in $(ls $DASTOOL/*.fa); do
    name=$(basename $i .fa)
    cp $i $DREP_BINS/$name".fasta"
done

cp $BR_BINS/*.fasta $DREP_BINS
```

Y ahora si, vamos a correr dRep ...

``` bash
nano src/09.drep_48h.slurm
```
...
#SBATHC -J drep
#SBATCH -t 0
#SBATCH -e outs/09.drep48h.err
#SBATCH -o outs/09.drep48h.out
#SBATCH --export=ALL
#SBATCH -n 10
#SBATCH -N 1
#SBATCH -p q2
...
dRep dereplicate results/09.drep/48h/ --debug -p 10 -comp 50 -con 10 -g results/09.drep/48h/bins/*.fasta
```
...
-   En la [carpeta compartida de Drive](https://docs.google.com/presentation/d/1f4vaJ_fV_8-i-dja2wtFx55vRkgeAB5Y4s-opCvFI1k/edit?usp=sharing) busquen la presentación para el Ejercicio 2, en la diapositiva correspondiente resuman sus resultados obtenidos para que los presenten.
